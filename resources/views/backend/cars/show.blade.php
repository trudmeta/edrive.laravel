    ...
    <div class="card-body">
        ...
        <x-backend.section-header>
            ...
            <x-slot name="toolbar">
                ...
                <a href='{{ route("backend.$models.index") }}' class="btn btn-secondary" data-toggle="tooltip" title="Cars List"><span>List</span></a>
                <x-buttons.edit route='{{ route("backend.$models.edit", $$model) }}' title="{{__('Edit')}} name" class="ms-1" icon="cil-settings" />
            </x-slot>
        </x-backend.section-header>
...
{{--                    <a href="{{ route('site.single.car', $car->alias) }}" class="btn btn-success" target="_blank"><i class="fas fa-link"></i> Public View</a>--}}
{{--                </div>--}}
                <div><h5>@lang('Characteristics')</h5></div>
                <ul>
                    @foreach($$model->getValuesSortedAttribute() as $values)
                    <li style="margin-bottom: 5px;">
                        @if ($values instanceof \Illuminate\Database\Eloquent\Model)
                            <p style="margin-bottom: 5px;">{{ $values->attribute->title }}</p>
                            {{ $values->title }}
                            <hr style="margin: 5px 0;">
                        @elseif ($values instanceof \Illuminate\Support\Collection)
                            <p>{{ $values->first()->attribute->title }}</p>
                            <ul>
